...
from legged_lab.assets.droid import x3_CFG
...
class x3RewardCfg(RewardCfg):
    ...
    ang_vel_xy_l2 = RewTerm(func=mdp.ang_vel_xy_l2, weight=-0.050)
    ...
    joint_deviation_hip_roll = RewTerm(func=mdp.joint_deviation_l1, weight=-0.3,params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_roll"])})
...
@configclass
class x3FlatAgentCfg(BaseAgentCfg):
    experiment_name: str = "x3_flat"
    wandb_project: str = "x3_flat"


@configclass
class x3RoughEnvCfg(x3FlatEnvCfg):
...
@configclass
class x3RoughAgentCfg(BaseAgentCfg):
    experiment_name: str = "x3_rough"
    wandb_project: str = "x3_rough"
    policy = RNNPolicyCfg()