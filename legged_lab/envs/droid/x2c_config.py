...
from legged_lab.assets.droid import X2C_CFG
...
@configclass
class X2CRewardCfg(RewardCfg):
    track_lin_vel_xy_exp = RewTerm(func=mdp.track_lin_vel_xy_yaw_frame_exp, weight=2.5, params={"std": 0.25})
    ...
    ang_vel_xy_l2 = RewTerm(func=mdp.ang_vel_xy_l2, weight=-0.14)
    ...
    body_orientation_l2 = RewTerm(func=mdp.body_orientation_l2,params={"asset_cfg": SceneEntityCfg("robot", body_names="pelvis_link")}, weight=-10.0)
    ...
    feet_too_near = RewTerm(func=mdp.feet_too_near_humanoid, weight=-5.0,params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*ankle_roll.*"]),"threshold": 0.3})
    ...
    joint_deviation_hip_yaw = RewTerm(func=mdp.joint_deviation_l1, weight=-1.5,params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_yaw"])})
    ...
    joint_deviation_legs = RewTerm(func=mdp.joint_deviation_l1, weight=-0.10,params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_pitch"])})
    ...
    feet_height = RewTerm(func=mdp.feet_height, weight=-5.0,params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*ankle_roll.*"]),"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=[".*ankle_roll.*"])})
@configclass
class X2CFlatEnvCfg(BaseEnvCfg):
    reward = X2CRewardCfg()

    def __post_init__(self):
        super().__post_init__()
        self.scene.height_scanner.prim_body_name = "pelvis_link"
        self.scene.robot = X2C_CFG
...
@configclass
class X2CFlatAgentCfg(BaseAgentCfg):
    experiment_name: str = "X2C_flat"
    wandb_project: str = "X2C_flat"


@configclass
class X2CRoughEnvCfg(X2CFlatEnvCfg):
...
@configclass
class X2CRoughAgentCfg(BaseAgentCfg):
    experiment_name: str = "X2C_rough"
    wandb_project: str = "X2C_rough"
    policy = RNNPolicyCfg()