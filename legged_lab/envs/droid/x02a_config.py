...
from legged_lab.assets.droid import X02A_CFG
...
class X02ARewardCfg(RewardCfg):
    ...
    # joint_deviation_arms = RewTerm(func=mdp.joint_deviation_l1, weight=-0.2, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_shoulder_roll.*", ".*_shoulder_yaw.*", ".*_elbow"])})
...
@configclass
class X02AFlatEnvCfg(BaseEnvCfg):

    reward = X02ARewardCfg()

    def __post_init__(self):
        super().__post_init__()
        self.scene.height_scanner.prim_body_name = "pelvis_link"
        self.scene.robot = X02A_CFG
...
@configclass
class X02AFlatAgentCfg(BaseAgentCfg):
    experiment_name: str = "x02a_flat"
    wandb_project: str = "x02a_flat"


@configclass
class X02ARoughEnvCfg(X02AFlatEnvCfg):
...
@configclass
class X02ARoughAgentCfg(BaseAgentCfg):
    experiment_name: str = "x02a_rough"
    wandb_project: str = "x02a_rough"
    policy = RNNPolicyCfg()
