...
from legged_lab.assets.droid import X03_CFG
...
class X03RewardCfg(RewardCfg):
    ...
    energy = RewTerm(func=mdp.energy, weight=-2.5e-3)
    ...
    feet_air_time = RewTerm(func=mdp.feet_air_time_positive_biped, weight=0.15, params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=".*ankle_pitch.*"), "threshold": 0.5})
    ...
    feet_too_near = RewTerm(func=mdp.feet_too_near_humanoid, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*ankle_pitch.*"]), "threshold": 0.3})
    feet_stumble = RewTerm(func=mdp.feet_stumble, weight=-2.0, params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=[".*ankle_pitch.*"])})
    dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-1.5)
    joint_deviation_waist_roll = RewTerm(func=mdp.joint_deviation_l1, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*waist_roll"])})
    joint_deviation_waist_yaw = RewTerm(func=mdp.joint_deviation_l1, weight=-10, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*waist_yaw"])})
    ...
    joint_deviation_hip_roll = RewTerm(func=mdp.joint_deviation_l1, weight=-0.10, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_roll"])})
    ...
    joint_deviation_legs = RewTerm(func=mdp.joint_deviation_l1, weight=-0.05, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_pitch", ".*_knee.*"])})
    joint_deviation_legs_ankle = RewTerm(func=mdp.joint_deviation_l1, weight=-0.15, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_ankle.*"])})
...
@configclass
class X03FlatEnvCfg(BaseEnvCfg):
...
@configclass
class X03FlatAgentCfg(BaseAgentCfg):
    experiment_name: str = "X03_flat"
    wandb_project: str = "X03_flat"


@configclass
class X03RoughEnvCfg(X03FlatEnvCfg):
...
@configclass
class X03RoughAgentCfg(BaseAgentCfg):
    experiment_name: str = "X03_rough"
    wandb_project: str = "X03_rough"
    policy = RNNPolicyCfg()
